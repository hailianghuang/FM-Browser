...
$(document).ready(function() {
    if (window.location.search) {
        ...
        /* Shiny.inputBindings.getBindings() return the InputBinding instances
           for every (native) input type that Shiny supports (selectInput, textInput, actionButton etc.)  */
...
if (window.location.search.match(/silent_download/g)){
  var checkExist = setInterval(function() {
   if ( document.getElementById("downloadPDF") && document.getElementById("downloadPDF").href.indexOf("downloadPDF") > -1 ) {
       window.open(document.getElementById("downloadPDF").href, "_top");
